...
class Ghost:
    ...
    def move_ghost(self):
        new_direction = random.choice([2, 3, 4, 1])
        if new_direction != self.direction and not self.turns[new_direction - 1]:
            self.direction = new_direction
        else:
            if not self.turns[self.direction - 1]:
                self.x_pos += self.speed if self.direction == 2 else -self.speed if self.direction == 1 else 0
                self.y_pos += self.speed if self.direction == 4 else -self.speed if self.direction == 3 else 0

        # Update center_x and center_y here
        self.center_x = self.x_pos + 22
        self.center_y = self.y_pos + 22
